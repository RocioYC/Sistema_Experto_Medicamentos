...
def mostrar_resultado_final(composicion_act, med_act_es, clase_act, best_n, best_s, best_j, 
                          efectos_sust, fuente_recomendacion, df_info):
    ...
    # Mostrar origen
    origen = "Sustituto directo prioritario" if fuente_recomendacion == "primera_linea" else "Alternativa terapéutica"
    print(f"\n🔍 Origen: {origen}")
...
def mostrar_analisis_detallado(composicion_act, med_act_es, review_act, clase_act, comp_principal, 
                             ...
                             fuente_recomendacion, df_info=None):
    ...
    try:
        ...
        print(f"   └─ Nombre: {str(best_n).upper()}")
        print(f"   └─ Justificación: {best_j}")
        
        print(f"\n📋 ORIGEN: {'Sustituto directo' if fuente_recomendacion == 'primera_linea' else 'Alternativa terapéutica'}")
